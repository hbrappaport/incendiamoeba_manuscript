#k-Tracked Particles, this script takes x,y positional data sequenced by 
#frame number and assigns particle IDs to track particles over time
#comments updated 9/19/25
#xpos and ypos outputs are in microns, no need for conversion 


#restart, this clears all local variables 
rm(list = ls())
...
#DO DATA IMPORT HERE################## under environment pick the csv/text file 
#that you output from FIJI/from Text(readr)

#use the filename that you saved and copy and paste down below to save that name
# NAME OF DATASET 
name <- "copy name here" 
df<-data
df$temp <- "25"  # name group in df, you don't have to change this but it's a parameter that
#changes in the other datasets

# Define parameters, look up how the k nearest neighbors algorithm works
...
df <- df %>%
  ...
  # filter out rows where stepwise displacement is greater than 10, this filters nonadherent cells
...
#see the trajectories, does this look right based on the original image sequence?
...
####WORKING DIRECTORY####
#check your working directory. make it the parent folder of the image set. 
#session/set working directory/get your parent folder
# output QC results, automatic output into your working directory 
...
# save velocity aspect ratio data, check this output, are there new columns? do they 
#show stepwise displacement and other variables?
